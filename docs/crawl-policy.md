...
Canonical-target findings are limited to targets already represented by durable evidence. Sitemap recommendation state remains separate and is never recomputed by the metadata-audit engine. Phase 21 consumes durable evidence for existing-sitemap comparison. Phase 22 consumes the parser-owned source-link projection plus durable page, redirect, and scope evidence without new network or parsing work. Phase 24 adds bounded parser-owned image occurrence evidence. Phase 25 adds bounded parser-owned JSON-LD, Microdata, and RDFa evidence without executing markup or fetching vocabulary contexts; keywords, content scoring, AI rewriting, and rich-result certification remain deferred.
...
Image discovery is parser-owned and bounded to `img`, `picture`/`source srcset`, and the documented `data-src`, `data-lazy-src`, `data-original`, and `data-srcset` allowlist. Phase 24 does not parse external stylesheets or select responsive candidates using browser layout. Repeated normalized resources are verified at most once. Internal verification must pass the existing DNS, SSRF, redirect, scope, timeout, response-size, proxy, and safe-logging controls. External images are inventoried but are not fetched by default. Data URLs are never fetched and only media type, length estimate, and a truncated fingerprint are retained.
