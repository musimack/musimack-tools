...
The Phase 25 private API is confined to `/api/internal/v1/audits/structured-data`; viewers inspect retained results with `runs.view`, while creation, execution, and export generation require `jobs.submit`. Six CSV inventories, complete JSON, and Markdown remain authenticated. See [`docs/structured-data-audit.md`](docs/structured-data-audit.md) and the [automated browser QA guide](docs/structured-data-audit-browser-qa.md).
