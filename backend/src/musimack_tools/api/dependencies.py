...
def permission_for_request(  # noqa: C901, PLR0911, PLR0912
    ...
    if "/site-audits" in path:
        ...
        if "/site-profiles" in path and (
            ...
            return Permission.SETTINGS_MANAGE
        if method == "GET" and (path.endswith("/presets") or "/site-profiles" in path):
            return Permission.JOBS_SUBMIT
        if method == "GET":
            return Permission.RUNS_VIEW
        if path.endswith(("/reconcile", "/rebuild-summary", "/archive")):
            return Permission.SETTINGS_MANAGE
    ...
    if "/audits/metadata" in path:
        ...
        if method == "POST" and path.endswith("/exports"):
            return Permission.ARTIFACTS_VIEW
        return Permission.RUNS_VIEW
    if "/audits/sitemaps" in path:
        if method == "POST" and path.endswith(("/sitemaps", "/discover", "/execute")):
            return Permission.JOBS_SUBMIT
        if method == "POST" and path.endswith("/exports"):
            return Permission.JOBS_SUBMIT
        return Permission.RUNS_VIEW
    if "/audits/links" in path:
        if method == "POST" and path.endswith(("/links", "/execute", "/exports")):
            return Permission.JOBS_SUBMIT
        return Permission.RUNS_VIEW
    if "/audits/internal-links" in path:
        if method == "POST" and path.endswith(("/internal-links", "/execute", "/exports")):
            return Permission.JOBS_SUBMIT
        return Permission.RUNS_VIEW
    if "/audits/images" in path:
        if method == "POST" and path.endswith(("/images", "/execute", "/exports")):
            return Permission.JOBS_SUBMIT
        return Permission.RUNS_VIEW
    if "/audits/structured-data" in path:
